...
| 45 | Nome "Distribuidor {code}" genérico em vez de nome real — fallback em 14 pontos do código | Quando `resolveDistName()` e fallbacks inline não encontram o CodDist em nenhuma fonte (NomeDist, DISTRIBUIDORAS_MAP, DISTRIBUIDORAS_DYNAMIC_CACHE), retornavam `` `Distribuidor ${code}` `` — parecia nome real de distribuidora, mas era só fallback. Item de referência em ruptura com CodDist não mapeado aparecia como "Distribuidor 503" no relatório. Fix: unificar todos os 15 pontos de fallback (server.ts x10, ConditionSelector.tsx x3, OrderReturnView.tsx x1, App.tsx x5, useManualSearch.ts x1, useDistributorWizards.ts x1) para "Distribuidor Não Identificado" — texto que deixa claro que o nome não foi resolvido. Investigação de `loadDistribuidoresFromAPI()` confirmou: sem bugs, CodDist pode genuinamente faltar na API SmartPed (limitação de dado externo) | server.ts, src/components/ConditionSelector.tsx, src/components/OrderReturnView.tsx, src/App.tsx, src/hooks/useManualSearch.ts, src/hooks/useDistributorWizards.ts |
| 46 | Input de quantidade perdia foco a cada tecla — re-render pesado no onChange | SwapsTable.tsx:1940 — input controlado ligado direto a `item.qtd`, chamando `onUpdateQty` (atualiza estado raiz + recalcula grupos/badges) a CADA TECLA via onChange. Re-render pesado entre uma tecla e outra fazia o campo perder foco no meio da digitação. Clique na seta nativa também disparava onChange causando deslocamento de layout (cor/tamanho do campo por causa do alerta de MÍNIMO COMERCIAL). Fix: mesmo padrão já usado em `alertInputs` (linhas 1778-1801) — estado local `qtyInputs` só pra digitação, `onUpdateQty` chamado só no `onBlur` (ou Enter). Estilo dinâmico (vermelho pulsante) continua baseado em `item.qtd` (valor confirmado), não no que está sendo digitado | src/components/SwapsTable.tsx |
| 47 | Crash "Cannot read properties of undefined (reading 'length')" no PendingOrdersTable | Condição de corrida entre fechar a modal de faturamento (que apaga `billedGroups`) e checagem assíncrona de retorno que ainda tenta atualizar a mesma entrada já removida. `next[g] = { ...prev[g], logs }` quando `prev[g]` é undefined vira `{ logs: [...] }` — sem `status` nem `faltas`. PendingOrdersTable lê `group.faltas.length` e quebra. Fix: (1) guard defensivo `group.faltas?.length ?? 0` no componente; (2) guards `if (!prev[g]) continue/return` nos 4 pontos de `setBilledGroups` em useBilling.ts (linhas 374, 621, 674, 737) | src/components/PendingOrdersTable.tsx, src/hooks/useBilling.ts |
| 48 | `onUpdateDistributor` / `overriddenDistributors` era código morto nunca invocado | Investigação de sessão seguinte confirmou que `onUpdateDistributor` (prop de SwapsTable) nunca era chamado por nenhum elemento de UI — `overriddenDistributors` só era resetado pra `{}`, nunca populado. O `overDist` em useOptimizationResult.ts:463 era sempre undefined — a lógica de limpar `motivoAcao`/`origem` nunca disparava. Mecanismo REAL de troca de distribuidor (ConditionSelector → handleSelectCondition) sempre resolveu codDist/codProdutoDist/codProduto corretamente, sem bug. Removido: prop `onUpdateDistributor` (App.tsx + SwapsTable.tsx), estado `overriddenDistributors` + lógica `overDist` (useOptimizationResult.ts), 4 resets `setOverriddenDistributors({})` (App.tsx). **NÃO reintroduzir** — se precisar de override de distribuidor, fazer via ConditionSelector existente | src/App.tsx, src/components/SwapsTable.tsx, src/hooks/useOptimizationResult.ts |
| 49 | Parsing de resposta de API externa sem conferir formato real — fetchComprasHistoricoBatch buscava `data.items`/`data.produtos` (não existem) em vez de `data.resultados[ean].resumo` (formato real do Ferramentinhas). Falha 100% silenciosa: endpoint retornava 200 com dado válido, parsing simplesmente não encontrava os campos e preenchia tudo com zero/vazio. Mesma família do bug #28 (falhas de escrita/leitura silenciosas). Fix: parsing reescrito pra `data.resultados` + `d.resumo.melhor_preco`. **Testado com dado real** (EANs 7896714257600, 7898148294121 — ambos retornaram melhorPreco > 0). Reforça regra: sempre validar parsing contra resposta real da API, não supor formato | server.ts |
| 50 | Filtro REF-FILTER de vendas/estoque/remoção de Referência removia EAN próprio do produto | `allEansForVendas` filtering (server.ts:~4598) usava `eanCatMap` (categorizado a partir de `similares`) pra remover EANs "marca". Mas `similares` é auto-inclusivo — retorna o próprio EAN do produto entre os candidatos. Referência/marca (ex: TROK G, EUROFARMA) aparecia na própria busca de similares, era categorizada como "marca", e o filtro removia o EAN DELE MESMO — perdia vendas/estoque/melhor preço pago do próprio item. Fix: `ownEans` (Set de todos os EANs originais do SICF) como exceção — `if (cat === "marca" && !ownEans.has(...))`. Mesma família da regra documentada em "4 listas de EAN independentes" | server.ts |
| 51 | Dispersar/Completar completamente quebrados desde a refatoração de 2026-08-14 (commit eba2f98) — nunca encontravam nenhum item elegível | Causa raiz: matching estático contra activeReport (1 linha por item, nunca duas distribuidoras pro mesmo item) — impossível de bater. Restaurada lógica original de busca ao vivo via /api/search-products (branch backup-tudo-funcionando), com regra de acréscimo máx 10% no Completar. 2º bug encontrado depois: botão "Dispersar" passa nome composto "Dist [Condicao \| Prazo]d" (chave que SwapsTable usa pra agrupar por condição/prazo), mas a busca comparava com nome puro em distributorGroupings — nunca batia. Fix: handleStartDispersingWizard filtra activeReport reproduzindo a mesma chave composta. Hook morto useDistributorWizards.ts (nunca importado, duplicava a lógica quebrada) deletado | src/App.tsx, src/hooks/useDistributorWizards.ts (removido) |
| 52 | external_suppliers não competia no fluxo de encomendas (/api/encomendas/buscar-ofertas-batch) — só competia no SICF e em "Ofertas do Dia" | Lógica de matching (EAN exato + fallback texto + resolução de preço/desconto) extraída de dentro de /api/optimize pra função compartilhada matchExternalSupplierOffer, reaproveitada nos dois endpoints. Fix adicional: preço de referência (melhorPrecoAtual, base pro cálculo de desconto%) estava sendo calculado sobre a lista de ofertas SmartPed NÃO filtrada — podia pegar preço de um substituto de EAN diferente (Condicoes/Molecula retorna Substitutos com EAN diferente do pesquisado). Corrigido pra filtrar por EAN igual + estoque > 0 antes do cálculo | server.ts |
| 53 | Itens "mantidos no original" (sem swap) às vezes apareciam como "Distribuidor Não Identificado" genérico mesmo quando o distribuidor era conhecido | resolveDistName(ref, codDistVar) era chamado ANTES de codDistVar ser atualizado com o CodDist real (ref.CodDist) — recebia sempre o valor antigo (0 na maioria dos casos, falsy), pulando a busca em DISTRIBUIDORAS_MAP/DISTRIBUIDORAS_DYNAMIC_CACHE. Fix: inverter a ordem (calcular CodDist antes de resolver o nome) em 2 pontos (bestOriginalDist/bestOriginalCodDist e originalDist/originalCodDist) | server.ts |
| 54 | Painel "Pedidos Monitorados" mostrava distribuidoras presas pra sempre em "Aguardando" mesmo com o pedido real já finalizado (ex: "Pan/Santa" aparecia junto de um distribuidor genuinamente pendente) | Em /api/faturar, itens sem codDist resolvido (0/undefined — legítimo quando nenhuma condição original nem substituto foi encontrado) eram silenciosamente atribuídos ao código 2 (DISTRIBUIDORAS_MAP[2] = "Pan/Santa") via `it.codDist \|\| 2`. Esse item fantasma nunca teve pedido real enviado, a SmartPed nunca retorna Status=3 pra ele — ficava preso pra sempre no rastreamento. Fix: `\|\| 0` em vez de `\|\| 2` nos 2 pontos dentro de /api/faturar (payload de resposta + insertPedidoMonitorado) | server.ts |
...
| 59 | Motor de otimização aceitava trocar medicamento puro por combinado (2+ princípios ativos) quando a embalagem tinha a mesma contagem de comprimidos | `validateSwapEquivalence` normalizava o texto removendo `+` e `.`/`,` antes de extrair dosagem — "50MG+12.5" virava "50MG 12 5", o segundo princípio ativo desaparecia da comparação. Fix: novo `COMBO_PATTERN` detecta `numero+separador(+/)+numero` no texto cru (antes da normalização) e rejeita se um lado é combinado e o outro não. Caso real: EAN 7896112134220 (Losartana 50mg 60cpr) vs EAN 7896112427933 (Losartana+Hidro 50mg+12.5mg 60cpr), mesma contagem, aceito incorretamente antes | swap-validation.ts |
| 60 | Item em ruptura total (sem estoque em nenhuma distribuidora) disparava o alerta "potencialmente fracionado" com botão OK, como se confirmar a quantidade resolvesse algo — mas não tinha nenhuma oferta real pra rotear o pedido | `calculateQuantityAlert` rodava incondicionalmente mesmo quando `originalDist` já tinha virado "Sem Estoque" duas linhas antes. Fix: pula o cálculo do alerta quando `originalDist === "Sem Estoque"`, retorna `alertaConfirmarQtd:false` direto. Caso real: LAMINA BARBEAR WILKINSON INOX 3UN (EAN 7891051015210), ruptura confirmada em todas as distribuidoras | server.ts |
| 61 | Busca manual (`/api/search-products`) podia mostrar produto combinado (ex: Losartana+HCT) sozinho ou antes do produto de composição pura buscada, ordenado só por preço | Ordenação não distinguia "bate exatamente com o que foi digitado" de "combinado que bateu no wildcard por coincidência textual". Fix: novo `matchTier` ("exato"/"similar") calculado por regex de combinação na descrição + detecção de intenção de combinado na query (`+`, `/`, ou 2+ dosagens) — combinados só ficam em primeiro se a query pedir combinação; preço continua sendo o desempate dentro de cada grupo | server.ts |
| 62 | "Olhinho" (Estoque & Similares) mostrava só 1-2 resultados incompletos pra item substituto promovido por ruptura, mesmo quando a composição tinha vários equivalentes reais em estoque | Item substituto (achado pela SmartPed nacional, nunca comprado por essa farmácia) não está cadastrado no ERP local — Ferramentinhas devolve vazio, sistema cai no fallback local pobre. Fix: se o EAN atual falhar, tenta de novo com o EAN ORIGINAL do SICF (que quase certamente está cadastrado localmente) antes do fallback. Também: resposta do fallback já tinha `regexUsed:true` mas nunca era usado — agora o modal mostra aviso visual de "dado pode estar incompleto" quando cai nesse caminho. Só aplicado onde existe EAN original disponível (SwapsTable) | src/components/EanEyeButton.tsx, SimilarProductsModal.tsx, SwapsTable.tsx, server.ts |
| 63 | "Completar Pedido" e "Dispersar Pedido" podiam sugerir puxar item pra outra distribuidora achando que era o mesmo produto mais barato, quando na verdade era um substituto por molécula da SmartPed (não validado) | Busca de oferta no distribuidor-alvo não usava `onlyExactEan:true`, misturando ofertas do EAN exato com substitutos genéricos do `Condicoes/Molecula` — sem passar pela validação de equivalência do motor principal. Fix: `onlyExactEan:true` nas duas chamadas, restringe a busca só às ofertas do EAN exato em cada distribuidor | src/App.tsx |
| 64 | Busca manual (`/api/search-products`) sem unidade de dosagem retornava produto de dosagem errada junto — ex: "gabapentina 400" (sem "mg") trazia comprimidos de 300mg misturados | `normalizeSearchQuery` tratava número solto no final da query como "quantidade de caixa" por padrão e removia do texto de busca, mesmo sem sinal nenhum de que essa era a intenção — sobrava só o nome do princípio ativo no wildcard, que bate com qualquer dosagem. Fix: só extrai quantidade quando o número vem explicitamente depois de "com"/"c/" na query crua (antes da remoção de stopwords) — sem esse separador, o número fica no texto de busca como parte da dosagem | server.ts |
...
| `allEansForVendas` | vendas/estoque/melhor-preço (batch Ferramentinhas) | REF-FILTER de Remoção de Referência (~4598) — removia EAN próprio; corrigido em #50 | **Sim** (ownEans, fix #50) |
...
> **Último deploy:** `smartped-cli-00107-5xq`.
