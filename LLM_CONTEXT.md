...
O projeto utiliza um repositório monolítico (Express + React). 

### Diretório Raiz
*   `server.ts`: **(CRÍTICO)** O núcleo do sistema. Contém o servidor Express. Abriga toda a lógica de parsing SICF, rotas de API (`/api/optimize`, `/api/faturar`, `/api/distribuidores`), chunking de requisições de EANs e a lógica matemática de seleção de melhor distribuidora.
...
*   `backend-tests.ts`: Suíte autônoma de auto-testes bloqueantes (`runEngineSelfTests`). Valida 5 cenários mandatórios de troca (Sabor, Dosagem, Quantidade, Cor e Equivalência Permitida). Se houver falha, encerra a execução com `process.exit(1)`, abortando o build de produção.
*   `package.json` e `vite.config.ts`: Configurações de build. O script `npm run build` executa primeiramente `tsx backend-tests.ts`, garantindo que nenhum código seja compilado em produção se os auto-testes falharem. O backend é compilado usando `esbuild` para um `server.cjs` autocontido na build de produção.
*   `API_TREE_TRIER.md`: **(REFERÊNCIA)** Árvore completa de consulta à API Trier (Ferramentinhas) — endpoints, parâmetros, estruturas de resposta, chaves de extração e fluxos.
*   `API_TREE_SMARTPED.md`: **(REFERÊNCIA)** Árvore completa de consulta à API SmartPed — endpoints, parâmetros, estruturas de resposta, fluxos de otimização e normalização de dados.

### Frontend (`src/`)
*   `App.tsx`: Orquestrador visual. Controla guias de navegação (Otimização, Retornos, Itens do Dia), exibe os relatórios gerados e os modais. É o grande gerenciador de estados globais.
