...
4.  **SeleÃ§Ã£o e Match (FunÃ§Ã£o `findBestSubstitute` no `server.ts`):** 
    ...
    *   **ExibiÃ§Ã£o e IntegraÃ§Ã£o do PMC (PreÃ§o MÃ¡ximo ao Consumidor) e NormalizaÃ§Ã£o Financeira:** Para dar transparÃªncia completa de precificaÃ§Ã£o e margem comercial para a farmÃ¡cia, o backend calcula e injeta os campos `originalPmc` e `novoPmc` no relatÃ³rio principal, bem como o PMC individual de cada alternativa, extraindo-os diretamente das respostas comerciais da SmartPed. Para garantir 100% de confiabilidade e tolerÃ¢ncia a falhas nos dados de ERPs ou de parceiros:
            ...
            - **Fallback Baseado no PreÃ§o de Tabela:** Se o PMC nÃ£o for explicitamente retornado ou vier zerado da API, o sistema emprega um fallback automÃ¡tico e preciso baseado no **PreÃ§o de Tabela** do produto (`precoTabela * 1.4`), em vez do preÃ§o lÃ­quido de compra. Isso impede que produtos com alto desconto tenham seu PMC drasticamente subestimado (evitando que um produto de PMC de R$ 41,96 e custo lÃ­quido de R$ 6,72 apareÃ§a de forma incorreta como R$ 9,41).
            - O frontend renderiza essas informaÃ§Ãµes de forma sofisticada e de alta visibilidade:
            - Na tabela principal de itens (`SwapsTable.tsx`), o PMC correspondente Ã© exibido sutilmente em fonte menor logo abaixo do PreÃ§o Base e do PreÃ§o LÃ­quido (sinalizado em verde esmeralda para destacar a oferta ativa).
            - No modal de intercambialidade e condiÃ§Ãµes comerciais (`InterchangeabilityModal.tsx`), o PMC de cada alternativa de compra ou laboratÃ³rio Ã© renderizado logo abaixo do seu respectivo PreÃ§o LÃ­quido na lateral direita, facilitando a tomada de decisÃ£o pelo comprador.
            - Na tela de adiÃ§Ã£o manual de itens (`App.tsx`), as ofertas pesquisadas exibem o PMC logo abaixo de seu respectivo PreÃ§o LÃ­quido na coluna correspondente, garantindo controle visual completo antes de enviar o item para o lote.
